...
void update_api(char *txn,double *acct){
    ...
    //printf("%d %d %.2lf %d %d\n",txn_id, txn_type, txn_amount,txn_acct1,txn_acct2);

    //txn reading complete
    //printf("lock_bro %d",txn_acct1);
    if(txn_type==1){
    ...
    else if(txn_type==4 && txn_acct1 != txn_acct2){
        //printf(" lock %d",txn_acct2);
...
void *update(void *arg){
    ...
    while(1){
	 ...
	 txn_ctr++;
         if(dataptr >= endptr || txn_ctr > num_txn){
